...
class ProjectCommand extends Command with TemplateHelpers{
  ...
  ProjectCommand(){
    argParser.addFlag('include-test', abbr: 't', help: 'Include testing framework in project?', defaultsTo: true);
    argParser.addFlag('use-conan', abbr: 'c', help: 'setup project with conan support?', defaultsTo: true);
    argParser.addOption('name',abbr: 'n', help: 'Name of the project to create');
    argParser.addOption('include-dir',abbr: 'i', help: 'Name of the include dircetory of the project', defaultsTo: 'include');
    argParser.addOption('src-dir',abbr: 's', help: 'Name of the source directory of the project', defaultsTo: 'src');
    argParser.addOption('test-dir',abbr: 'T', help: 'Name of the test directory for the project', defaultsTo: 'test');
    argParser.addOption('test-include-dir',abbr: 'I', help: 'Name of the include dircetory of the project', defaultsTo: 'include');
    argParser.addOption('test-src-dir',abbr: 'S', help: 'Name of the source directory of the project', defaultsTo: 'src');
    argParser.addOption('test-framework', abbr: 'F', help: 'Testing Framework to Use',
                        allowed: ['gTest', 'catch'], defaultsTo: 'catch');
    argParser.addOption('project-type', abbr: 'p', help: 'Type of project to create',
                        allowed: ['exe', 'lib', 'dll'], defaultsTo: 'exe');                    
  ...
  void run() async {
    ...
    // create espada file
    await File('${project.name.snakeCase}/.espada').writeAsStringSync(jsonEncode(project));
  ...
  Future createTestProjects(Project project) async {
    ...
    // create test folder
      var test_dir = await Directory("${project.name.snakeCase}/${project.test_dir}").create(recursive: true);
    // create test_dir
      var test_include_dir = await Directory("${project.name.snakeCase}/${project.test_dir}/${project.test_include_dir}").create(recursive: true);
      var test_src_dir = await Directory("${project.name.snakeCase}/${project.test_dir}/${project.test_src_dir}").create(recursive: true);
    // download test tools
      var catch_url = 'https://raw.githubusercontent.com/catchorg/Catch2/master/single_include/catch2/catch.hpp';
      var catch_file = "${project.name.snakeCase}/${project.test_dir}/${project.test_include_dir}/catch.hpp";
      createFileFromURL(project, catch_file, catch_url );
      catch_url = 'https://raw.githubusercontent.com/eranpeer/FakeIt/master/single_header/catch/fakeit.hpp';
